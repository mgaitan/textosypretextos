...
- `npm run dev` recompila assets, reconstruye datos derivados y levanta `zola serve` sin drafts.
- `npm run dev:drafts` hace lo mismo pero incluye borradores.
- `npm run build` reconstruye assets, `data/site_index.json` y genera `public/`.
...
Eso genera un Markdown con front matter TOML compacto alineado al sitio.
...
- `autores/`: estas páginas conservan título, bio, foto y género; no deben listar artículos.
- `etiquetas/`: estas páginas conservan título y metadata editorial mínima; no deben listar artículos.
- `buscar.md`: página funcional del buscador.

Las listas de artículos por autor, etiqueta y subsección se derivan desde los
artículos con:

```bash
uv run scripts/build_zola_index.py
```

El build lo corre automáticamente antes de `zola build`.
...
- `authors`
- `tags`
- `description` — resumen corto para listados y metadatos sociales
...
- `[extra].hero_alt`
- `[extra].subtitle`
- `[extra].deck` — epígrafe del artículo; soporta markdown. Un blockquote
...
- Las etiquetas se tratan como case-insensitive: si sólo cambia la mayúscula,
  debe ser el mismo tag.
...
- No usar `<br>` manuales en contenido nuevo si el bloque completo puede resolverse con `poetry`.
